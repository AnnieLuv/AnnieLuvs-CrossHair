...
void MenuWindow::OnPaint(HDC& hdc, HWND hwnd)
{
	...
	if (hwnd == hWnd) {
		...
		offscreengraphics->DrawString(L"Cross Center Gap", -1, &font, PointF(trackbar_rects[5].X, trackbar_rects[5].Y - 23), &brush_pinkgrad);

		offscreengraphics->DrawString(L"Annie Luv's CrossHair <3\nCoded entirely in C++", -1, &font, PointF(220, 330), &brush_red);
...
void MenuWindow::OnDrawItem(DRAWITEMSTRUCT * drawitemstruct)
{
	...
	else if (drawitemstruct->hwndItem == hwnd_button_minimize) {
		itemoffscreengraphics->DrawString(L"-", -1, &font, PointF(5, 8), &brush_gradred);
	}
	else if (drawitemstruct->hwndItem == hwnd_button_close) {
		itemoffscreengraphics->DrawString(L"X", -1, &font, PointF(5, 8), &brush_gradred);
	}

	if (drawitemstruct->hwndItem == hwnd_button_minimize || drawitemstruct->hwndItem == hwnd_button_close) {
		itemoffscreengraphics->DrawRectangle(&pinkpen, 0, 0, button_dim_minclose - 3, button_dim_minclose - 3);
	}
	else
...
LRESULT CALLBACK MenuWindow::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE: {
		...
		pmenu_window->hCursor = LoadCursor(0, IDC_ARROW);
		...
		pmenu_window->CreateButton(hWnd, pmenu_window->hwnd_button_minimize, "-", pmenu_window->button_rects[4]);
		pmenu_window->CreateButton(hWnd, pmenu_window->hwnd_button_close, "x", pmenu_window->button_rects[5]);
	...
	case WM_COMMAND: {
		...
		else if (hwnd == pmenu_window->hwnd_button_minimize)
			ShowWindow(pmenu_window->hWnd, SW_MINIMIZE);

		else if (hwnd == pmenu_window->hwnd_button_close)
			PostQuitMessage(0);
